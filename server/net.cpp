...
int unicast_net::send(char *buffer, char *servIP, unsigned short size)
{
	...
	if (sendto(sock, buffer, size, 0, (struct sockaddr *) &udpServAddr, sizeof(udpServAddr)) == size) return size;
...
int unicast_net::reciv(char *buffer,unsigned long *IP_Sender,unsigned short size) {
	...
	if ((sSize = recvfrom(sock, buffer, size, 0, (struct sockaddr*)&udpClntAddr, (socklen_t*)&ucleng)) < 0) {
