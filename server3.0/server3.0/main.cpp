...
//globalna tablica zawieraj¹ca wszystkich u¿ytkowników i dane o nich
...
netObjects* netObjectsToPass;

char addres[] = "192.168.0.220";
char* servAddr = addres;
...
{
	netObjects* netObjectsSent = (netObjects*)ptr;  // wskaŸnik do obiektu klasy unicast_net
	unicast_net* reciver = netObjectsSent->reciver;
	unicast_net* sender = netObjectsSent->sender;
	Frame frame;

	while (true) {
		unsigned long buffer;
		int frame_size = reciver->reciv((char*)&frame, &buffer, sizeof(Frame));   // oczekiwanie na nadejœcie ramki 
		...
		if (frame_size > 0) {
			...
			//wysy³anie info o zmianie gry do ka¿dego klienta
...
void InteractionInitialisation()
{
	...
	uni_reciv = new unicast_net(10002);      // obiekt do odbioru ramek sieciowych
	uni_send = new unicast_net(10001);       // obiekt do wysy³ania ramek

	netObjectsToPass = new netObjects;
	netObjectsToPass->reciver = uni_reciv;
	netObjectsToPass->sender = uni_send;
	...
	threadReciv = CreateThread(
		...
		(void *)netObjectsToPass,               // argument to thread function
...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu avg_cycle_time
		...
		sprintf(text, "server id: %d (%0.0f fps  %0.2fms) ", my_car->iID, fFps, 1000.0 / fFps);
	...
	int       nCmdShow)
{
	...
	main_window = CreateWindow(class_name, "server - wersja b", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
